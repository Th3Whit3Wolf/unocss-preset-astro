 ...
 */
const mixColor = (
  color1: string | CSSColorValue,
  color2: string | CSSColorValue,
  weight: string | number
): CSSColorValue | undefined => {
  const colors = [color1, color2];
  const cssColors: CSSColorValue[] = [];
  for (let c = 0; c < 2; ++c) {
    const color = (
      typeof colors[c] === "string"
        ? parseCssColor(colors[c] as string)
        : colors[c]
    ) as CSSColorValue | undefined;
 ...
 */
const tint = (color: string | CSSColorValue, weight: string | number) =>
  mixColor("#fff", color, weight);

/**
 * Mix color with black. @see {@link mixColor}
 */
const shade = (color: string | CSSColorValue, weight: string | number) =>
  mixColor("#000", color, weight);
